...
being run using printTimeDetails which takes input for i (current loop) n (final loop) and st(starting time using time.time() before the loop started
"""
def printTimeDetails(i,n,st):
    dt = time.time()-st
    sys.stdout.write(str(round(float(i)/n*100,3))+
		     '% complete | Elapsed: '+
	             str(datetime.timedelta(seconds = round(dt)))+' | ETA: '+
